...
int Inventory::use_equipped(Entity& target, int attacker_x, int attacker_y, int target_x, int target_y) {
    if (!equipped_item)
        throw NoWeaponEquippedException();
    return equipped_item->use_item(target, attacker_x, attacker_y, target_x, target_y);
}
