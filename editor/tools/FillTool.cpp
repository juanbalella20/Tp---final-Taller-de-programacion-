...
std::vector<CellChange> FillTool::on_press(const Map& map, int layer,
                                           int x, int y, int active_gid) {
    if (!map.in_bounds(x, y)) return {};

    int target = map.get_cell(layer, x, y);
    if (target == active_gid) return {};  // ya es del color destino: nada que rellenar
    ...
    while (!stack.empty()) {
        ...
        if (map.get_cell(layer, cx, cy) != target) continue;
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (map.get_cell(layer, nx, ny) != target) continue;
            visited[ny][nx] = true;
